...
const ru: Translations = {
    ...
    api: {
        ...
        featureEmailsDisabled: 'Информационные письма в настоящее время отключены.',
    ...
    emails: {
        ...
        changelogPreview: 'Узнайте о последних обновлениях ClawHost: {{title}}',
        changelogHeading: 'Что нового',
        changelogButton: 'Посмотреть журнал изменений',
        changelogVisitButton: 'Перейти в ClawHost',
        changelogFooter:
    ...
    createClaw: {
        ...
        providerAtCapacity:
            'Из-за высокого спроса серверы этого провайдера временно закончились. Мы активно работаем над решением этой проблемы.',
    ...
    changelog: {
        ...
        release14Title: 'Миграция на Hetzner, партнёрская система и новые языки',
    ...
    admin: {
        ...
        failedToLoadUsersDescription: 'Произошла ошибка при загрузке пользователей. Попробуйте снова.',
    ...
    affiliate: {
        ...
        noPaymentsDescription: 'Когда приглашённые пользователи совершат покупки, их платежи появятся здесь.'
    },
    affiliateProgram: {
        title: 'Партнёрская программа',
        description: 'Узнайте, как работает партнёрская программа ClawHost, сколько вы можете заработать и правила участия.',
        lastUpdated: 'Последнее обновление: 1 апреля 2026 г.',
        overviewTitle: '1. Обзор',
        overviewText: 'Партнёрская программа ClawHost позволяет вам зарабатывать вознаграждения, приглашая новых пользователей в ClawHost. Когда кто-то совершает покупку после посещения ClawHost по вашей реферальной ссылке, вы получаете комиссию с его платежей. Программа бесплатна и доступна всем зарегистрированным пользователям ClawHost.',
        howItWorksTitle: '2. Как это работает',
        howItWorksText: 'Начать работу с партнёрской программой просто:',
        howItWorksStep1: 'Зарегистрируйте аккаунт ClawHost. Уникальный реферальный код генерируется для вас автоматически.',
        howItWorksStep2: 'Поделитесь своей реферальной ссылкой с друзьями, коллегами или вашей аудиторией. Ваша ссылка имеет формат: clawhost.cloud?ref=YOUR_CODE.',
        howItWorksStep3: 'Когда кто-то совершает покупку после посещения ClawHost по вашей ссылке, это регистрируется как ваш реферал.',
        howItWorksStep4: 'Вы получаете комиссию каждый раз, когда ваш приглашённый пользователь совершает соответствующую покупку.',
        earningsTitle: '3. Заработок и выплаты',
        earningsText: 'Вот как работает партнёрский заработок:',
        earningsCommission: 'Вы получаете комиссию 15% с каждой соответствующей покупки, совершённой вашими приглашёнными пользователями. Комиссии распространяются на планы ClawHost Cloud и ClawHost Go.',
        earningsMonthly: 'Для месячных подписок вы получаете комиссии в течение 1 года с даты реферала.',
        earningsYearly: 'Для годовых подписок вы получаете комиссию только за первый год.',
        earningsPayout: 'Минимальная сумма вывода составляет 100 $ USD. Чтобы запросить вывод средств, свяжитесь с нашей службой поддержки.',
        earningsPaymentMethod: 'Выплаты обрабатываются через PayPal. При запросе выплаты необходимо указать действительный адрес электронной почты PayPal.',
        earningsCurrency: 'Весь заработок рассчитывается и отображается в USD.',
        referralCodeTitle: '4. Ваш реферальный код',
        referralCodeText: 'Каждый пользователь получает уникальный реферальный код при регистрации. Вы можете настроить его один раз, чтобы сделать более запоминающимся:',
        referralCodeUnique: 'Ваш реферальный код уникален для вашего аккаунта и не может быть передан другому пользователю.',
        referralCodeOneChange: 'Вы можете настроить свой реферальный код ровно один раз. Выбирайте внимательно — это изменение постоянно и не может быть отменено.',
        referralCodeFormat: 'Реферальные коды могут содержать только буквы, цифры, дефисы и подчёркивания.',
        referralWindowTitle: '5. Окно атрибуции рефералов',
        referralWindowText: 'Реферал закрепляется за вами на 3 месяца с момента, когда приглашённый пользователь впервые посещает ClawHost по вашей ссылке. Если приглашённый пользователь не совершает покупку в течение этого 3-месячного окна, реферал истекает и комиссия не будет начислена. Если пользователь переходит по другой реферальной ссылке, новый реферал заменяет предыдущий.',
        eligibilityTitle: '6. Право на участие',
        eligibilityText: 'Для участия в партнёрской программе вы должны соответствовать следующим требованиям:',
        eligibilityAccount: 'У вас должен быть зарегистрированный аккаунт ClawHost.',
        eligibilityStanding: 'Ваш аккаунт должен быть в хорошем состоянии без истории нарушений правил.',
        eligibilityAge: 'Вам должно быть не менее 18 лет или возраст совершеннолетия в вашей юрисдикции.',
        rulesTitle: '7. Правила программы',
        rulesText: 'Для поддержания целостности партнёрской программы действуют следующие правила:',
        rulesNoSelfReferral: 'Самореферальность строго запрещена. Вы не можете приглашать свои собственные аккаунты или аккаунты, которыми вы управляете.',
        rulesNoFakeAccounts: 'Создание поддельных аккаунтов, автоматизированных регистраций или использование ботов для генерации рефералов запрещено.',
        rulesNoSpam: 'Отправка нежелательных массовых сообщений (спама) для продвижения вашей реферальной ссылки не допускается.',
        rulesNoMisrepresentation: 'Вы не можете искажать информацию о ClawHost, его услугах или партнёрской программе каким-либо образом.',
        rulesNoIncentivized: 'Предложение прямых денежных стимулов (например, оплата пользователям за регистрацию по вашей ссылке) не допускается.',
        terminationTitle: '8. Нарушение и прекращение',
        terminationText: 'Любое нарушение этих правил приведёт к немедленной потере всех ожидающих и заработанных вознаграждений. ClawHost оставляет за собой право приостановить или навсегда заблокировать ваш аккаунт в партнёрской программе. В серьёзных случаях ваш аккаунт ClawHost также может быть закрыт. Все решения относительно нарушений являются окончательными.',
        marketingTitle: '9. Как продвигать',
        marketingText: 'Существует множество креативных и законных способов поделиться вашей реферальной ссылкой и увеличить доход:',
        marketingSocial: 'Делитесь ссылкой в социальных сетях, таких как X, LinkedIn, Reddit и Facebook. Напишите о своём опыте с ClawHost и включите реферальную ссылку.',
        marketingBlog: 'Пишите статьи в блоге, руководства или обзоры о ClawHost. Естественно включайте реферальную ссылку в контент.',
        marketingVideo: 'Создавайте видеоконтент на YouTube или TikTok, демонстрируя, как вы используете ClawHost для развёртывания и управления ИИ-агентами.',
        marketingCommunity: 'Участвуйте в сообществах разработчиков, форумах и серверах Discord. Когда кто-то спрашивает об облачном хостинге или развёртывании ИИ-агентов, рекомендуйте ClawHost со своей ссылкой.',
        marketingNewsletter: 'Если вы ведёте рассылку или список электронной почты, упомяните ClawHost в соответствующем выпуске с вашей реферальной ссылкой.',
        marketingComparison: 'Пишите честные сравнительные статьи или руководства, которые подчёркивают, чем ClawHost отличается от других платформ.',
        changesToProgramTitle: '10. Изменения в программе',
        changesToProgramText: 'ClawHost оставляет за собой право изменять, приостанавливать или прекращать партнёрскую программу в любое время без предварительного уведомления. Это включает изменения комиссионных ставок, окон рефералов, порогов выплат и правил программы. Продолжение участия после изменений означает принятие обновлённых условий.',
        getStartedTitle: '11. Начать',
        getStartedText: 'Готовы начать зарабатывать? Перейдите в свою партнёрскую панель, чтобы получить реферальную ссылку и начните делиться ею со своей сетью.',
        getStartedButton: 'Перейти в партнёрскую панель',
        contactTitle: '12. Контакт',
        contactText: 'Если у вас есть вопросы о партнёрской программе, нужна помощь с реферальным кодом или вы хотите сообщить о нарушении, свяжитесь с нами по адресу'
    }
} as const
