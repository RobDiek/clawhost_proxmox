...
async function main() {
  ...
  vmConn.on('ready', () => {
    console.log('Connected to Hermes VM. Running CPU/RAM check...')
    
    vmConn.exec('echo "=== CPU CORES ==="; nproc; lscpu | grep -E "^CPU\(s\):|Core\(s\) per socket:|Socket\(s\):"; echo "=== MEMORY ==="; free -h', (err, stream) => {
      if (err) throw err
