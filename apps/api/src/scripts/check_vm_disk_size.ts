...
async function main() {
  ...
  vmConn.on('ready', () => {
    console.log('Connected to Hermes VM. Running disk checks...')
    
    vmConn.exec('echo "=== LSBLK ==="; lsblk; echo "=== DF -H ==="; df -h; echo "=== FDISK ==="; sudo fdisk -l /dev/sda || sudo fdisk -l /dev/vda || true', (err, stream) => {
      if (err) throw err
