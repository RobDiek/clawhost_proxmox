...
async function main() {
  ...
  vmConn.on('ready', () => {
    console.log('Connected to Hermes VM. Checking installation status...')
    
    vmConn.exec('echo "--- HERMES INSTALL LOG ---"; tail -n 50 /var/log/hermes-install.log; echo "--- SYSTEMD STATUS ---"; systemctl status hermes-gateway || true', (err, stream) => {
      if (err) throw err
