...
const fetchExistingSlugs = async (token: string): Promise<string[]> => {
    const res = await fetch(
        github.CONTENTS(BLOG_PATH, BASE_BRANCH),
...
const generateBlogPost = async (c: Context) => {
    ...
    if (!baseSha) {
        return fail(c, 'Failed to get Production branch SHA', 500)
    }
    ...
    if (!branchCreated) {
        return fail(c, 'Failed to create branch', 500)
    }

    const pushed = await pushToGitHub(githubToken, slug, fullContent, branchName)

    if (!pushed) {
        return fail(c, 'Failed to push blog post to branch', 500)
    }

    const prUrl = await createPullRequest(githubToken, branchName, topic.title, slug)

    if (!prUrl) {
        return fail(c, 'Failed to create pull request', 500)
    }

    const merged = await mergePullRequest(githubToken, prUrl)

    if (!merged) {
        return ok(c, { slug, title: topic.title, pr: prUrl, merged: false }, 'Blog post PR created but not merged.')
    }

    await deleteBranch(githubToken, branchName)

    return ok(c, { slug, title: topic.title, pr: prUrl, merged: true }, 'Blog post generated, merged, and branch cleaned up.')
}
