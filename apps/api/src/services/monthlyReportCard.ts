...
async function compose(data: ReportCardData, apiKey: string | undefined): Promise<string> {
    ...
    const prompt = `אתם אנליסט שיווק בכיר. כתבו "כרטיס תוצאות חודשי" בעברית (פנייה בלשון רבים), Markdown, על בסיס הנתונים בלבד — בלי להמציא מספרים. מבנה קבוע:
## כרטיס תוצאות — ${data.monthLabel}
1. **סטטוס כללי**: שורה אחת עם אימוג'י מצב — 🟢 על המסלול / 🟡 פיגור / 🔴 קריטי — ונימוק קצר.
2. **יעד מול בפועל** — טבלה עם 5 עמודות בדיוק: מדד | יעד | בפועל | פער % | מצב. בעמודת "מצב" שימו אימוג'י לפי גודל הפער: 🟢 כשהפער עד 10%, 🟡 כשהפער 10-25%, 🔴 כשהפער מעל 25% (או יעד שהוחמץ לחלוטין). כסו: הכנסה/לקוחות (אם יש יעד), Ads (spend/ROAS/conversions), אורגני (תנועה/מילות מפתח/ציטוטי AI). אם אין נתון — כתבו "אין מדידה" ומצב ⚪.
3. **מה עבד** (2-3 נקודות מבוססות מספרים).
4. **מה לא עבד / סיכונים** (2-3).
5. **3 פעולות מומלצות לחודש הקרוב** — בדיוק שלוש, כטבלה: פעולה | אחראי | תאריך יעד. הפעולות נגזרות מהפערים האדומים/הצהובים (לדגום מנצחים, לתקן ירידות, לסגור פערי keyword/AI). אחראי = שם תפקיד/סוכן; תאריך יעד = תאריך קונקרטי בחודש הקרוב.
