...
async function resolveSiteUrl(instanceId: string, agentId?: string | null): Promise<string | null> {
    try { const { readResearchData } = await import('./agentContext'); const rd: any = (await readResearchData({ id: agentId } as any, instanceId)) || {}; const u = rd?.answers?.website || rd?.answers?.site || rd?.paidProfile?.siteUrl; if (u) return String(u) } catch { /* */ }
