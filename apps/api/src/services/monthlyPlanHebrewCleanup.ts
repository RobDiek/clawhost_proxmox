...
6. **כלל-על**: כל מילה או ביטוי באנגלית שאינם ברשימת הקיצורים/מותגים המותרת (סעיף 4) — חייבים תרגום מלא לעברית. דוגמאות: "audit"→"אודיט", "Content decay"→"דעיכת תוכן", "carousel"→"סבב", "traffic loss"→"אובדן תנועה", "low-effort high-impact"→"מאמץ נמוך והשפעה גבוהה", "baseline"→"בסיס", "tutorial"→"הדרכה", "how-to"→"מדריך", "quick win"→"רווח מהיר". אסור להשאיר מילת-פרוזה אנגלית בודדת. אם מונח מקצועי חייב להישאר באנגלית — כתוב את התרגום העברי ואחריו את המונח בסוגריים, לדוגמה: "פאנל ידע (Knowledge Panel)".

7. **טוקנים מכניים** (key=value, word_count<300, records[], נתיבים עם נקודות כמו internal_seo_audit.records) — הסר לחלוטין או נסח מחדש כעברית טבעית; לעולם אל תשאיר אותם בטקסט.
