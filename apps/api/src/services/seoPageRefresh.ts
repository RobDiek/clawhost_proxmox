...
- 2-4 כותרות משנה H2 בפורמט שאלה אמיתית (כמו שמשתמשים מקלידים בגוגל), שמכסות זוויות ש**עוד לא** מכוסות בעמוד.
- **תשובה-קודם:** המשפט/פסקה הראשונה תחת כל כותרת עונה ישירות ובמלואו על השאלה (כ-60-160 מילים), ורק אחר כך הרחבה — כך מנועי תשובות (AI) מצטטים את הקטע.
- **ערך ייחודי (information gain):** פרטים פרקטיים, מספרים, דוגמאות מהשטח, ניסיון אמיתי — לא מילוי וניסוח מחדש של מה שכבר קיים. האורך נובע מכיסוי מלא של הכוונה, לא מיעד מילים.
- מקטע "שאלות נפוצות" בסוף (H2 "שאלות נפוצות", שאלות ב-H3, תשובה ישירה בתחילת כל תשובה).
- עברית בלבד (חוץ משמות מותג). טון מקצועי, ידידותי, לשון רבים. בלי דחיסת מילות מפתח.
...
- הוסיפו 2-4 כותרות משנה H2 בפורמט שאלה אמיתית (כמו שמשתמשים מקלידים בגוגל).
- **תשובה-קודם:** תחת כל כותרת, המשפט/פסקה הראשונה עונה ישירות על השאלה (כ-60-160 מילים), ואז הרחבה — כך מנועי תשובות מצטטים את הקטע.
- **ערך ייחודי (information gain):** העמיקו עם פרטים פרקטיים, מספרים, דוגמאות וניסיון אמיתי — לא מילוי וניסוח מחדש. האורך נובע מכיסוי הכוונה, לא מיעד מילים.
- מקטע "שאלות נפוצות" בסוף (3-5 שאלות, H2 "שאלות נפוצות" ואז H3, תשובה ישירה בתחילת כל תשובה).
- עברית בלבד (חוץ משמות מותג רשמיים). טון ענייני, מקצועי, לשון רבים. בלי דחיסת מילות מפתח.
...
export async function applyEditTimeSeo(
    ...
    try {
        ...
        // Freshness signal (2026): we just substantively edited the page, so stamp
        // a real dateModified. Never fabricate datePublished (unknown here).
        primaryNode.dateModified = new Date().toISOString()
