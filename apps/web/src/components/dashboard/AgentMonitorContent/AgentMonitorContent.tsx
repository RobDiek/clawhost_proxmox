...
import { ChartLineUpIcon, CpuIcon, HardDriveIcon, ClockIcon, GaugeIcon } from '@phosphor-icons/react'
...
const AgentMonitorContent: FC<AgentMonitorContentProps> = ({ agentId, readOnly }): ReactNode => {
    const { data: liveData, isPending, isError } = useAgentMetrics(agentId, !readOnly)
    ...
    if (isError) return (
        <div className='flex h-full items-center justify-center p-5'>
            <PanelPlaceholder
                icon={<ChartLineUpIcon className='text-muted-foreground h-6 w-6' weight='duotone' />}
    ...
    const memColor = getUsageColor(memPercent, { high: '#ef4444', medium: '#f59e0b', low: '#3b82f6' })
    const diskColor = getUsageColor(data.disk.usagePercent, { high: '#ef4444', medium: '#f59e0b', low: '#8b5cf6' })

    return (
        <div className='h-full space-y-4 overflow-y-auto p-5'>
            <div className='flex items-center justify-between'>
                <div className='flex items-center gap-2'>
                    <h3 className='text-sm font-medium'>{t('clawDetail.metricsTitle')}</h3>
                    <LiveBadge />
                </div>
                {data.uptime && (
            ...
            <MetricCard title={t('clawDetail.metricsMemory')} icon={<GaugeIcon className='h-4 w-4' style={{ color: memColor }} />}>
                <UsageBar value={memPercent} color={memColor} label={`${memPercent}%`} detail={`${formatBytes(data.memory.used)} / ${formatBytes(data.memory.total)}`} />
                {memHistory.length > 1 && <MonitorChart data={memHistory} color={memColor} label='MEM' />}
            </MetricCard>
            <MetricCard title={t('clawDetail.metricsDisk')} icon={<HardDriveIcon className='h-4 w-4' style={{ color: diskColor }} />}>
                <UsageBar value={data.disk.usagePercent} color={diskColor} label={`${data.disk.usagePercent}%`} detail={`${formatBytes(data.disk.used)} / ${formatBytes(data.disk.total)}`} />
            </MetricCard>
            <MonitorNetworkCard rxBytes={data.network.rxBytes} txBytes={data.network.txBytes} />
            <MonitorLoadAvgChart load1={data.loadAvg.load1} load5={data.loadAvg.load5} load15={data.loadAvg.load15} />
