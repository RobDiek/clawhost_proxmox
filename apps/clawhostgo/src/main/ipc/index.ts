...
const registerAllHandlers = (): void => {
    ...
    ipcMain.handle('oauth-window', (_event: unknown, url: string, callbackPrefix: string, title: string) => {
        return new Promise((resolve, reject) => {
            ...
            win.webContents.on('did-finish-load', async () => {
                try {
                    const currentUrl: string = await win.webContents.executeJavaScript('location.href')
    ...
    ipcMain.handle('oauth-github-exchange', async (_event: unknown, code: string) => {
